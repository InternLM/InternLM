...
def exam_hybrid_zero_optim_with_ckpt_load_save(args):
    # init
    rank, world_size, zero_parallel, overlap_sync_param, overlap_sync_grad= args
    config.parallel.zero1 = zero_parallel
    config.hybrid_zero_optimizer.overlap_sync_param = overlap_sync_param
    config.hybrid_zero_optimizer.overlap_sync_grad = overlap_sync_grad
    ...
    for zero1_param, zero2_param in \
        zip(zero_optimizer._fp32_flat_param_groups_of_current_rank.values(), \
    ...
    for zero1_param, zero2_param in \
        zip(zero_optimizer._fp16_param_groups.values(), \
            zero_optimizer2._fp16_param_groups.values()):
            loose_close(zero1_param, zero2_param)
    
    

zero_parallel_check_list = [-1, 1, 4] #[-1, 1, 4]
overlap_sync_grad_check_list = [True, False]
overlap_sync_param_check_list = [False] #[True, False]
miro_num_check_list = [1, 2, 4] #[1, 2, 4]

# 首先需要实现一个正确的 pytorch ddp，并且要支持 no_sync
# 先不考虑带 tp 和 pp 的情况
# case1: 基本正确性测试，单机8卡
...
def test_hybrid_zero_optim_with_ddp(zero_parallel, overlap_sync_param, overlap_sync_grad, micro_num):
    ...
    with ctx.Pool(processes=8) as pool:
        pool.map(exam_hybrid_zero_optim_with_ddp, [[rank, 8, zero_parallel, overlap_sync_param, 
                                                    overlap_sync_grad, micro_num] for rank in range(8)])
        pool.close()
        pool.join()


# # case2: ckpt保存和加载可以完全恢复之前的状态
@pytest.mark.parametrize("zero_parallel", zero_parallel_check_list)
@pytest.mark.parametrize("overlap_sync_param", overlap_sync_param_check_list)
@pytest.mark.parametrize("overlap_sync_grad", overlap_sync_grad_check_list)
def test_hybrid_zero_optim_with_ckpt_load_save(zero_parallel, overlap_sync_param, overlap_sync_grad):
    ctx = mp.get_context("spawn")
    with ctx.Pool(processes=8) as pool:
        pool.map(exam_hybrid_zero_optim_with_ckpt_load_save, [[rank, 8, zero_parallel, overlap_sync_param, 
                                                                overlap_sync_grad] for rank in range(8)])
        pool.close()
        pool.join()


if __name__ == '__main__':
    pytest.main(['-s', '-q', 'test_optimizer.py']) #'-m embedding',