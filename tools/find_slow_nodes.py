...
class DLCJobInfo:
    ...
    def do_request(self, api_product, api_query, api_method, api_path):
        ...
        resp = requests.request(
            method=api_method, url=api_url, params=api_query, headers=headers, verify=False, timeout=10
        )
...
class ProcessFilter:
    """
    filter slow nodes which ib bandwidth is not normal.
    """
    ...
    def do_all_reduce_multi_node(self):
        ...
        while count <= self.iters:
            ...
            if count > self.warmup:
                bw_sum += bw
        self.nodeinfo.ib_bw = bw_sum / (self.iters - self.warmup)
        if self.rank == 0:
            printflock(
                f"RANK {self.rank} in HOST {self.nodeinfo.hostname} has outer busBw {self.nodeinfo.ib_bw} GB/s",
                flush=True,
            )
    ...
    def do_all_reduce_single_node(self):
        ...
        while count <= self.iters:
            ...
            if count > self.warmup:
                bw_sum += bw
            count += 1
        self.nodeinfo.nvlink_bw = bw_sum / (self.iters - self.warmup)
        printflock(
            f"RANK {self.rank} in HOST {self.nodeinfo.hostname} has internal busBw {self.nodeinfo.nvlink_bw} GB/s",
            flush=True,
        )
        dist.destroy_process_group()
    ...
    def get_bus_bandwidth(self, cost_seconds: float, ntasks: int):
        # print(f"self.buffer_count: {self.buffer_count}, " + \
        # f"self.type_size: {self.type_size}, cost_seconds: {cost_seconds}, ntasks: {ntasks}")
    ...
    def split_node_groups(self, node_rank_groups: List):
        ...
        if number_nodes % 2 == 0:
            left_group, right_group = (
                node_rank_groups[: int(number_nodes / 2)],
                node_rank_groups[int(number_nodes / 2) :],
            )
            return left_group, right_group
        else:
            left_group, right_group = (
                node_rank_groups[: int(number_nodes / 2) + 1],
                node_rank_groups[int(number_nodes / 2) + 1 :],
            )
            return left_group, right_group
...
if __name__ == "__main__":
    ...
    if args.gethost:
        ...
        ncclfilter = ProcessFilter(
            launcher=args.launcher,
            ib_threshold=args.ib_threshold,
            nvlink_threshold=args.nvlink_threshold,
