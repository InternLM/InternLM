...
- ``save_ckpt_folder``: 检查点存储路径，参数类型 ``str``，在开启检查点存储功能时为必选参数

- ``checkpoint_every``: 检查点存储频率，参数类型 ``int``

- ``load_ckpt_folder``: 初始化检查点/权重加载路径。参数类型 ``str``，默认为 ``None``，详见 :ref:`load-ckpt-folder`

- ``async_upload``: 是否开启异步上传。详见 :ref:`asyncupload`

- ``async_upload_tmp_folder``: 异步上传临时存储路径。

- ``oss_snapshot_freq``: 快照存储频率。详见 :ref:`snapshot`

- ``auto_resume``: 是否开启检查点自动恢复。详见 :ref:`autoresume`

- ``stop_file_path`` : 检查点存储控制文件的路径.详见 :ref:`stopfile`
